...
建立可运行、可测试、职责清晰的前后端 TypeScript 工程，并锁定公共请求、响应、页面状态和错误合同。

## 对应范围

- Goals：G-1、G-3、G-4
- Success Criteria：SC-1、SC-7、SC-8
- Requirements：FR-1—FR-3、FR-10、FR-15—FR-18、NFR-1、NFR-2、NFR-4、NFR-6
...
- 生成 DTO、模板 ID、页面状态和统一错误类型。
- 最小健康检查与 Mock 默认配置。

### Out of Scope

- 具体模板提示词、生成编排、完整 UI 和真实模型调用。
...
- [ ] 创建 `shared` 类型包，导出四个模板 ID、`GenerateRequest`、`GeneratedCopy`、`GeneratedPage`、`GenerateResponse` 和内部 `VisualPlan`。
